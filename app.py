...
session_state = st.session_state #session state to store the FamilyExpenseTracker object

#check if the FamilyExpenseTracker object is already created
if "expense_tracker" not in session_state:
    session_state["expense_tracker"] = FamilyExpenseTracker()

st.markdown("<h1 style='text-align: center; color: #f63366;'>Family Expense Tracker</h1>", unsafe_allow_html=True)
...
if selected == "Data Entry":
    ...
    with st.expander("Add Family Member"):
        ...
        if st.button("Add Member"):
            try:
                ...
                member = [
                    member
                    for member in expense_tracker.family_members
    ...
    if not expense_tracker.family_members:
        st.info(
        ...
        total_expenditure = expense_tracker.calculate_total_expenses()         # Calculate total expenditure
    ...
    expense_data = [
        (expense.category, expense.value) for expense in expense_tracker.expenses
    ]
    if expense_data:
